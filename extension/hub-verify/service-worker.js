...
chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  (async()=>{
    ...
    if(type==='BLOXD_GLOBAL_REGASSERT'){
      ...
      const alreadyUploaded=uploadedKeys.includes(key);
      ...
      let next=recent;
      if(!existing){total+=1;next=[{key,raw,seenAt:Date.now(),frameUrl:String(sender?.url||'').slice(0,500),uploadStatus:alreadyUploaded?'uploaded':'pending'},...recent].slice(0,MAX_RECENT);}
      ...
      try{
        ...
        const updated=await setState({
          ...
          latestUpload:{ok:true,code:result.code||code||null,playerName:result.playerName||null,pendingVerification:result.pendingVerification||null,seenAt:now}
        });
        await badge();
        sendResponse({
          ok:true,
