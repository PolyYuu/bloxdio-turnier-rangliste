...
const DEFAULT_STATE={
  ...
  latestUpload:null,
  relayMode:'passive-global-relay',
  uploadEnabled:true
};
...
chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  (async()=>{
    ...
    if(type==='BLOXD_EXTENSION_READY'){
      const state=await setState({bloxdSeenAt:Date.now(),relayMode:'passive-global-relay',uploadEnabled:true});await badge();sendResponse({ok:true,state});return;
    }
    ...
    if(type==='BLOXD_REGISTRATION_CODE'){
      ...
      try{
        ...
        else sendResponse({ok:true,state,silent:true});
      }catch(_){sendResponse({ok:true,state,silent:true});}
      return;
    }
  ...
  })().catch((error)=>{console.error('[HUB Verify Passive Relay]',error);try{sendResponse({ok:false,silent:true,error:String(error?.message||error)});}catch(_){}});
  return true;
});
