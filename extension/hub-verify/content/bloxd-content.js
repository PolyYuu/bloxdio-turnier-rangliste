...
function hideTechnicalMarker(node){
  try{
    ...
    for(let depth=0;depth<3&&el;depth++,el=el.parentElement){
      ...
      if(text.includes(REGASSERT_START)&&text.length<4500){
...
async function init(){
  ...
  // Do not scan the existing chat history on startup. Only new messages/events
  // are relevant; persisted World-Code outbox entries will arrive as mutations.
}
