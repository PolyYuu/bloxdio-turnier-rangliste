(() => {
  ...
  const COPY = {
    de: {pending:'VERIFIZIERUNG AUSSTEHEND',pendingName:'PENDING',verification:'BLOXD VERIFIZIERUNG',statsHidden:'Statistiken noch nicht sichtbar',statsText:'Deine Statistiken, dein Rating und deine bisherigen Turniere werden sichtbar, sobald du verifiziert wurdest. Bis dahin hat dein HUB-Account noch keinen Spielernamen.',viewStatus:'VERIFIZIERUNGSSTATUS ANSEHEN',yourAccount:'DEIN ACCOUNT',overviewTitle:'Verifizierung ausstehend',overviewStrong:'Stats und Spielername werden nach der Bloxd-Verifizierung sichtbar.',overviewText:'Rangliste, Cups und öffentliche Spielerprofile kannst du bereits ganz normal ansehen.',viewProfile:'MEIN PROFIL ANSEHEN',statusTitle:'Verifizierungsstatus anzeigen',blocked:'Diese Funktion wird nach deiner Bloxd-Verifizierung freigeschaltet.',loginHint:'Nutze deinen aktuellen Bloxd.io Ingame-Namen oder deinen 8-Zeichen-Code.',loginLabel:'Ingame-Name oder 8-Zeichen-Code'},
    en: {pending:'VERIFICATION PENDING',pendingName:'PENDING',verification:'BLOXD VERIFICATION',statsHidden:'Statistics are not visible yet',statsText:'Your statistics, rating and previous tournaments become visible after verification. Until then, your HUB account does not have a player name yet.',viewStatus:'VIEW VERIFICATION STATUS',yourAccount:'YOUR ACCOUNT',overviewTitle:'Verification pending',overviewStrong:'Stats and player name become visible after Bloxd verification.',overviewText:'You can already use the ranking, cups and public player profiles normally.',viewProfile:'VIEW MY PROFILE',statusTitle:'View verification status',blocked:'This feature becomes available after your Bloxd verification.',loginHint:'Use your current Bloxd.io ingame name or your 8-character code.',loginLabel:'Ingame name or 8-character code'},
    fr: {pending:'VÉRIFICATION EN ATTENTE',pendingName:'EN ATTENTE',verification:'VÉRIFICATION BLOXD',statsHidden:'Statistiques pas encore visibles',statsText:'Tes statistiques, ton classement et tes anciens tournois deviennent visibles après la vérification. Jusque-là, ton compte HUB n’a pas encore de nom de joueur.',viewStatus:'VOIR LE STATUT DE VÉRIFICATION',yourAccount:'TON COMPTE',overviewTitle:'Vérification pending',overviewStrong:'Les statistiques et le nom du joueur apparaissent après la vérification Bloxd.',overviewText:'Tu peux déjà utiliser normalement le classement, les cups et les profils publics.',viewProfile:'VOIR MON PROFIL',statusTitle:'Voir le statut de vérification',blocked:'Cette fonction sera disponible après ta vérification Bloxd.',loginHint:'Utilise ton nom Bloxd.io actuel ou ton code à 8 caractères.',loginLabel:'Nom en jeu ou code à 8 caractères'}
  };
  ...
  function readCache(){try{const cached=JSON.parse(localStorage.getItem(CACHE_KEY)||'null');if(!cached||cached.status!=='pending')return null;const authId=storedAuthUserId();if(cached.auth_user_id&&authId&&cached.auth_user_id!==authId)return null;return cached;}catch(_){return null;}}
  function writeCache(data,authUserId){try{localStorage.setItem(CACHE_KEY,JSON.stringify({status:'pending',auth_user_id:authUserId||storedAuthUserId()||'',source:data?.source||data?.verification_source||'',cached_at:Date.now()}));}catch(_){}}
  ...
  function installHeader(){
    $('#hubPendingHeaderStatus')?.remove();const account=$('#loginDemoButton');if(!account)return;account.removeAttribute('data-i18n');account.textContent=pendingName();account.classList.add('is-account');account.disabled=true;account.setAttribute('aria-disabled','true');account.title=t('pending');
  ...
  function patchLoginModal(){
    const m=$('#v3AuthModal');if(!m)return;
    const form=$('#v3AuthForm',m);if(!form)return;
    const labels=$$('label',form);
    const first=labels[0];if(!first)return;
    const span=$('span',first);if(span)span.textContent=t('loginLabel');
    const p=$('p',m);if(p)p.textContent=t('loginHint');
  }
  ...
  async function refresh(){try{const session=await api.currentSession();if(!session){clearCache();return;}const{data,error}=await api.client.rpc('get_my_registration_status');if(error)throw error;if(data?.status==='verified'){clearCache();if(active)location.reload();return;}if(data?.status!=='pending')return;state=data;live.registrationState=data;writeCache(data,session?.user?.id);applyUi();}catch(err){console.warn('Pending HUB state could not be refreshed',err);}}
  function interceptPendingAction(e){if(!currentPendingState())return;const profileRoute=e.target.closest?.('[data-route="profile"], [data-hub-pending-profile]');if(profileRoute){e.preventDefault();e.stopImmediatePropagation();renderPendingProfile();return;}if(e.target.closest?.('[data-hub-pending-status]')){e.preventDefault();e.stopImmediatePropagation();location.href='pending.html';return;}const sensitive=e.target.closest?.('[data-v3-friend-add],[data-friend-add],[data-v3-friend-remove],[data-v3-friend-accept],[data-v3-friend-decline],[data-open-register],.next-cup-cta,[data-register-next-cup],[data-next-cup-register],#editAvatarButton,#renameButton');if(sensitive){e.preventDefault();e.stopImmediatePropagation();core.toast?.(t('blocked'),true);}}
  ...
  document.addEventListener('hub:auth-restored',(e)=>{const incoming=e.detail?.registrationState;if(incoming?.status==='pending'){state=incoming;live.registrationState=incoming;writeCache(incoming,e.detail?.session?.user?.id||e.detail?.user?.id);applyUi();return;}if(incoming?.status==='verified')clearCache();setTimeout(refresh,30);});
  const authObserver=new MutationObserver(()=>patchLoginModal());authObserver.observe(document.body,{childList:true,subtree:true});
  if(currentPendingState())applyUi();setTimeout(refresh,40);setInterval(refresh,12000);
})();
