(() => {
  ...
  const COPY = {
    de: {
      ...
      overviewStrong:'Stats und Spielername werden nach der Bloxd-Verifizierung sichtbar.',
      overviewText:'Rangliste, Cups und öffentliche Spielerprofile kannst du bereits ganz normal ansehen.',viewProfile:'MEIN PROFIL ANSEHEN',
      statusTitle:'Verifizierungsstatus anzeigen',blocked:'Diese Funktion wird nach deiner Bloxd-Verifizierung freigeschaltet.',
      myProfile:'Mein Profil',changePassword:'Passwort ändern',logout:'Log out',passwordTitle:'Passwort ändern',newPassword:'Neues Passwort',
      confirmPassword:'Passwort wiederholen',savePassword:'PASSWORT SPEICHERN',cancel:'ABBRECHEN',passwordShort:'Das Passwort muss mindestens 8 Zeichen lang sein.',
      passwordMismatch:'Die Passwörter stimmen nicht überein.',passwordSaved:'Passwort wurde geändert.',passwordError:'Passwort konnte nicht geändert werden.',
    ...
    en: {
      ...
      overviewStrong:'Stats and player name become visible after Bloxd verification.',overviewText:'You can already use rankings, cups and public player profiles normally.',
      viewProfile:'VIEW MY PROFILE',statusTitle:'View verification status',blocked:'This feature becomes available after your Bloxd verification.',
      myProfile:'My Profile',changePassword:'Change Password',logout:'Log out',passwordTitle:'Change Password',newPassword:'New password',
      confirmPassword:'Repeat password',savePassword:'SAVE PASSWORD',cancel:'CANCEL',passwordShort:'Password must be at least 8 characters long.',
      passwordMismatch:'Passwords do not match.',passwordSaved:'Password changed.',passwordError:'Password could not be changed.',
    ...
    fr: {
      ...
      overviewStrong:'Les statistiques et le nom du joueur apparaissent après la vérification Bloxd.',overviewText:'Tu peux déjà utiliser normalement le classement, les cups et les profils publics.',
      viewProfile:'VOIR MON PROFIL',statusTitle:'Voir le statut de vérification',blocked:'Cette fonction sera disponible après ta vérification Bloxd.',
      myProfile:'Mon profil',changePassword:'Changer le mot de passe',logout:'Se déconnecter',passwordTitle:'Changer le mot de passe',newPassword:'Nouveau mot de passe',
      confirmPassword:'Répéter le mot de passe',savePassword:'ENREGISTRER',cancel:'ANNULER',passwordShort:'Le mot de passe doit contenir au moins 8 caractères.',
  ...
  function addStyles(){
    ...
    style.textContent=`
      ...
      body.hub-pending-account .site-header [aria-label*="friend" i],body.hub-pending-account .site-header [title*="friend" i],body.hub-pending-account .site-header [aria-label*="freund" i],body.hub-pending-account .site-header [title*="freund" i]{display:none!important}
      ...
      .hub-pending-mini-info,.hub-pending-info{display:inline-grid;place-items:center;border:1px solid currentColor;border-radius:50%;font-family:Georgia,serif;font-style:italic;font-weight:800;line-height:1}
      .hub-pending-mini-info{width:15px;height:15px;font-size:9px}.hub-pending-info{width:32px;height:32px;color:#ffc65c;background:rgba(255,198,92,.08);font-size:14px}
  ...
  function renderPendingProfile(navigate=false){
    ...
    if(navigate)showPage('profile');
    if(location.hash!=='#profile'&&!navigate)return false;
    const code=accountCode();
    const title=$('#profilePlayerName');if(title)title.textContent=code;
    ...
    $('[data-hub-pending-status]')?.addEventListener('click',()=>{location.href='pending.html';},{once:true});
    return true;
  }

  function renderOverviewPendingCard(){
    const card=$('.my-rank-card');if(!card)return;
    card.dataset.pendingRendered=accountCode();
    card.innerHTML=`<header class="panel-header tight"><div><span class="eyebrow">${t('yourAccount')}</span><h2>${t('overviewTitle')}</h2></div><button class="hub-pending-info" type="button" data-hub-pending-status aria-label="${t('statusTitle')}" title="${t('statusTitle')}">i</button></header><div class="hub-pending-overview"><strong>${t('overviewStrong')}</strong><p>${t('overviewText')}</p><button class="secondary-button" type="button" data-hub-pending-profile>${t('viewProfile')}</button></div>`;
  }

  function hidePendingOnlyControls(){
    $$('[data-open-register],.next-cup-cta,[data-register-next-cup],[data-next-cup-register]').forEach(el=>{el.hidden=true;el.setAttribute('aria-hidden','true');el.style.setProperty('display','none','important');});
    const header=$('.site-header')||document;
    $$('#communityButton,#friendsButton,[data-open-friends],[data-friends-button],[aria-label*="friend" i],[title*="friend" i],[aria-label*="freund" i],[title*="freund" i]',header).forEach(el=>{el.hidden=true;el.setAttribute('aria-hidden','true');el.style.setProperty('display','none','important');});
  }

  function ensureAccountMenu(){
    let menu=$('#hubPendingAccountMenu');
    if(menu)return menu;
    menu=document.createElement('div');menu.id='hubPendingAccountMenu';menu.hidden=true;
    document.body.appendChild(menu);
    return menu;
  }
  function positionAccountMenu(){
    const account=$('#loginDemoButton'),menu=$('#hubPendingAccountMenu');if(!account||!menu||menu.hidden)return;
    const r=account.getBoundingClientRect();const w=Math.max(205,menu.offsetWidth||205);let left=Math.min(window.innerWidth-w-12,Math.max(12,r.right-w));let top=Math.min(window.innerHeight-(menu.offsetHeight||150)-12,r.bottom+8);menu.style.left=`${left}px`;menu.style.top=`${Math.max(12,top)}px`;
  }
  ...
  function openAccountMenu(){
    ...
    menu.innerHTML=`<button type="button" data-hub-account-profile>${t('myProfile')}</button><button type="button" data-hub-account-password>${t('changePassword')}</button><button type="button" class="danger" data-hub-account-logout>${t('logout')}</button>`;
    menu.hidden=!menu.hidden;if(!menu.hidden)positionAccountMenu();
  }

  function ensurePasswordModal(){
    let modal=$('#hubPendingPasswordModal');if(modal)return modal;
    modal=document.createElement('div');modal.id='hubPendingPasswordModal';modal.hidden=true;
    modal.innerHTML=`<div class="hub-pending-password-card"><h2 data-hub-password-title></h2><label><span data-hub-password-new></span><input type="password" autocomplete="new-password" data-hub-password-one></label><label><span data-hub-password-confirm></span><input type="password" autocomplete="new-password" data-hub-password-two></label><p class="hub-pending-password-message" data-hub-password-message></p><div class="hub-pending-password-actions"><button type="button" class="secondary-button" data-hub-password-cancel></button><button type="button" class="primary-button" data-hub-password-save></button></div></div>`;
    document.body.appendChild(modal);return modal;
  }
  function openPasswordModal(){
    closeAccountMenu();const modal=ensurePasswordModal();modal.hidden=false;
    $('[data-hub-password-title]',modal).textContent=t('passwordTitle');$('[data-hub-password-new]',modal).textContent=t('newPassword');$('[data-hub-password-confirm]',modal).textContent=t('confirmPassword');$('[data-hub-password-cancel]',modal).textContent=t('cancel');$('[data-hub-password-save]',modal).textContent=t('savePassword');
    $('[data-hub-password-one]',modal).value='';$('[data-hub-password-two]',modal).value='';const msg=$('[data-hub-password-message]',modal);msg.textContent='';msg.classList.remove('ok');setTimeout(()=>$('[data-hub-password-one]',modal)?.focus(),50);
  }
  async function savePassword(){
    const modal=ensurePasswordModal(),one=$('[data-hub-password-one]',modal)?.value||'',two=$('[data-hub-password-two]',modal)?.value||'',msg=$('[data-hub-password-message]',modal);msg.classList.remove('ok');
    if(one.length<8){msg.textContent=t('passwordShort');return;}if(one!==two){msg.textContent=t('passwordMismatch');return;}
    try{const{error}=await api.client.auth.updateUser({password:one});if(error)throw error;msg.textContent=t('passwordSaved');msg.classList.add('ok');setTimeout(()=>{modal.hidden=true;},900);}catch(err){console.error(err);msg.textContent=t('passwordError');}
  }
  async function logoutPending(){
    closeAccountMenu();try{await api.client.auth.signOut();}catch(err){console.warn(err);}clearCache();state=null;active=false;live.registrationState=null;location.href='index.html#overview';location.reload();
  }

  function installHeader(){
    const account=$('#loginDemoButton');if(!account)return;
    account.removeAttribute('data-i18n');account.textContent=accountCode();account.classList.add('is-account');account.disabled=false;account.removeAttribute('aria-disabled');account.title=t('yourAccount');
    let badge=$('#hubPendingHeaderStatus');if(!badge){badge=document.createElement('span');badge.id='hubPendingHeaderStatus';badge.className='hub-pending-header-status';account.insertAdjacentElement('afterend',badge);}badge.innerHTML=`<span class="hub-pending-mini-info">i</span> ${t('pending')}`;
  }

  function patchLoginModal(){
    const m=$('#v3AuthModal');if(!m)return;const form=$('#v3AuthForm',m);if(!form)return;
    const labels=$$('label',form);const first=labels[0];const label=first?.querySelector('span')||first;const intro=$('p',m);
    if(label)label.textContent=t('loginLabel');if(intro)intro.textContent=t('loginIntro');
  }
  ...
  function applyUi(){
    ...
    try{
      active=true;live.registrationState=state;setAccountStateClass('pending');document.body.classList.add('hub-pending-account');addStyles();hidePendingOnlyControls();installHeader();renderOverviewPendingCard();if(location.hash==='#profile')renderPendingProfile(false);patchLoginModal();
    }finally{applying=false;}
  }
  function queueReapply(){
    if(!active||reapplyQueued)return;reapplyQueued=true;requestAnimationFrame(()=>{reapplyQueued=false;applyUi();});
  }
  function clearPendingUi(){
    state=null;active=false;live.registrationState=null;document.body.classList.remove('hub-pending-account');$('#hubPendingHeaderStatus')?.remove();closeAccountMenu();$('#hubPendingPasswordModal')?.remove();
    const account=$('#loginDemoButton');if(account){account.disabled=false;account.removeAttribute('aria-disabled');account.removeAttribute('title');account.classList.remove('is-account');}
  }

  async function refresh(){
    try{
      const session=await api.currentSession();
      if(!session){clearCache();clearPendingUi();setAccountStateClass('logged-out');patchLoginModal();return;}
      const{data,error}=await api.client.rpc('get_my_registration_status');if(error)throw error;
      if(data?.status==='pending'){
        state=data;live.registrationState=data;writeCache(data,session?.user?.id);applyUi();return;
      }
      clearCache();const wasPending=active;clearPendingUi();setAccountStateClass(data?.status==='verified'?'verified':'verified');if(wasPending)location.reload();
    }catch(err){console.warn('HUB account state could not be refreshed',err);if(currentPendingState())applyUi();}
  }
  ...
  function intercept(e){
    ...
    if(target.closest?.('[data-hub-password-cancel]')){e.preventDefault();ensurePasswordModal().hidden=true;return;}
    if(target.closest?.('[data-hub-password-save]')){e.preventDefault();savePassword();return;}
    if(target===ensurePasswordModal()&&!ensurePasswordModal().hidden){ensurePasswordModal().hidden=true;return;}
  ...
  document.addEventListener('click',e=>{if(e.target.closest?.('[data-lang],#languageButton,#loginDemoButton'))setTimeout(()=>{patchLoginModal();if(active)applyUi();},0);},true);
  ...
  window.addEventListener('hashchange',()=>{if(currentPendingState()){if(location.hash==='#profile')renderPendingProfile(false);else applyUi();}});
  window.addEventListener('storage',()=>{if(active)applyUi();});
  document.addEventListener('hub:auth-restored',(e)=>{
    const detail=e.detail||{};
    if(detail.loggedIn===false){clearCache();clearPendingUi();setAccountStateClass('logged-out');return;}
    ...
    if(incoming?.status==='verified'){clearCache();const wasPending=active;clearPendingUi();setAccountStateClass('verified');if(wasPending)location.reload();return;}
    setTimeout(refresh,30);
  });

  observer=new MutationObserver(()=>queueReapply());
