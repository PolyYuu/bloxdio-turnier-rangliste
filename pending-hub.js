(() => {
  ...
  const COPY = {
    en: {
      ...
      viewStatus:'VIEW VERIFICATION STATUS', overviewTitle:'VERIFICATION PENDING',
      overviewText:'Your account has been created. You need to wait until your account is finally verified before you can view your stats or participate in tournaments.',
      viewProfile:'GO TO FINAL VERIFICATION',
      ...
      pendingIntro:'Your HUB account has been created successfully. Your player name, stats, tournament history and friends become visible after verification.',
    ...
    de: {
      ...
      viewStatus:'VERIFIZIERUNGSSTATUS ANSEHEN', overviewTitle:'VERIFIZIERUNG AUSSTEHEND',
      overviewText:'Dein Account wurde angelegt. Du musst warten, bis dein Account final verifiziert wurde, bevor du deine Stats einsehen und an Turnieren teilnehmen kannst.',
      viewProfile:'ZUR FINALEN VERIFIZIERUNG',
      ...
      pendingIntro:'Dein HUB-Account ist erfolgreich angelegt, dein Spielername, deine Stats, Turnierhistorie und Freunde werden erst nach der Verifizierung sichtbar.',
    ...
    fr: {
      ...
      viewStatus:'VOIR LE STATUT DE VÉRIFICATION', overviewTitle:'VÉRIFICATION EN ATTENTE',
      overviewText:'Ton compte a été créé. Tu dois attendre la vérification finale avant de pouvoir consulter tes statistiques ou participer aux tournois.',
      viewProfile:'VERS LA VÉRIFICATION FINALE',
      ...
      pendingIntro:'Ton compte HUB a bien été créé. Ton nom de joueur, tes statistiques, ton historique de tournois et tes amis seront visibles après la vérification.',
  ...
  function writeCache(data, userId) {
    try {
      ...
      localStorage.setItem(CACHE_KEY, JSON.stringify({
        ...
        auth_user_id:id,
        source:data?.source || '',
        code:cleanCode(data?.code || readCache()?.code),
        cached_at:Date.now()
      }));
    } catch (_) {}
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      /* Loader bar is intentionally close to the visible logo width. */
      html.hub-live-booting .site-header .hub-brand::before{content:""!important;visibility:visible!important;position:absolute!important;left:50%!important;bottom:-25px!important;transform:translateX(-50%)!important;width:46%!important;height:4px!important;border-radius:999px!important;background:rgba(255,255,255,.1)!important;box-shadow:inset 0 0 0 1px rgba(255,255,255,.035)!important}
      html.hub-live-booting .site-header .hub-brand::after{content:""!important;visibility:visible!important;position:absolute!important;left:27%!important;bottom:-25px!important;width:calc(var(--hub-load-progress,0) * .46%)!important;height:4px!important;border-radius:999px!important;background:linear-gradient(90deg,#37ead7,#56c7ff,#aa55ff)!important;box-shadow:0 0 16px rgba(67,224,221,.38)!important;transition:width .26s cubic-bezier(.2,.8,.2,1)!important}
      ...
      #hubPendingOverviewSurface{gap:13px;align-content:start;box-sizing:border-box;width:100%;padding:22px 24px 20px}
      ...
      .hub-pending-route-copy{min-width:0}
      .hub-pending-route-badge{display:inline-flex;align-items:center;gap:7px;padding:6px 9px;border-radius:999px;background:rgba(244,189,79,.11);color:#f4bd4f;font:900 9px/1 Montserrat;letter-spacing:.08em}
      .hub-pending-route-badge i{display:grid;place-items:center;width:14px;height:14px;border:1px solid currentColor;border-radius:50%;font:italic 800 8px Georgia}
      .hub-pending-route-copy h1{margin:8px 0;font:900 clamp(32px,5vw,52px)/1 Montserrat,Arial;letter-spacing:.12em;overflow-wrap:anywhere}
      .hub-pending-route-copy p{max-width:900px;margin:0;color:#a79bc8;font:600 13px/1.6 Montserrat,Arial}
      .hub-pending-route-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:16px}
      .hub-pending-route-card{padding:24px}
      .hub-pending-route-card .eyebrow{display:block;margin-bottom:5px;color:#f4bd4f;font-size:9px;font-weight:900;letter-spacing:.13em}
      .hub-pending-route-card .eyebrow.teal{color:#38f0db}
      .hub-pending-route-card h2{margin:6px 0 12px;font-size:28px;letter-spacing:-.03em}
      .hub-pending-route-card p{color:#a79bc8;line-height:1.65;font-size:13px}
      .hub-pending-route-code{display:grid;gap:10px;margin:17px 0;padding:17px;border:1px solid rgba(255,255,255,.06);border-radius:12px;background:rgba(255,255,255,.035)}
      .hub-pending-route-code>span{color:#81758f;font-size:8px;font-weight:900;letter-spacing:.13em}
      .hub-pending-route-code>strong{font-size:30px;letter-spacing:.18em;word-break:break-all}
      .hub-pending-route-actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:5px}
      .hub-pending-route-status{min-height:0;margin:10px 0 0!important;font-size:11px!important}
      .hub-pending-route-status:empty{display:none!important}
  ...
  function hideRestricted() {
    $$('[data-open-register],.next-cup-cta,[data-register-next-cup],[data-next-cup-register]').forEach(el => {
      el.hidden = true;
      el.style.setProperty('display','none','important');
    });
    const header = $('.site-header') || document;
    $$('#communityButton,#friendsButton,[data-open-friends],[data-friends-button]', header).forEach(el => {
      el.hidden = true;
      el.style.setProperty('display','none','important');
    });
  }
  ...
  function renderOverview() {
    ...
    surface.innerHTML = `<span class="hub-pending-overview-kicker">HUB ACCOUNT</span><h2>${t('overviewTitle')}</h2><p>${t('overviewText')}</p><button type="button" class="secondary-button" data-hub-pending-status-open>${t('viewProfile')}</button>`;
  }
  ...
  function renderPendingPage(page = $('#hubPendingPage')) {
    ...
    page.innerHTML = `<div class="hub-pending-page-inner">
      <section class="hub-pending-route-hero">
        <div class="hub-pending-route-avatar">•••</div>
        <div class="hub-pending-route-copy">
          <span class="hub-pending-route-badge"><i>i</i>${t('pending')}</span>
          <h1>${code}</h1>
          <p>${t('pendingIntro')}</p>
        </div>
      </section>
      ...
      <div class="hub-pending-route-grid">
        <article class="hub-pending-route-card">
          <span class="eyebrow">${t('registration')}</span>
          <h2>${t('pendingTitle')}</h2>
          <p>${t('pendingText')}</p>
          <div class="hub-pending-route-code">
            <span>${t('yourCode')}</span>
            <strong>${code}</strong>
            <div class="hub-pending-route-actions">
              <button type="button" class="secondary-button" data-hub-copy-code>${t('copyCode')}</button>
              <button type="button" class="secondary-button" data-hub-check-status>${t('checkStatus')}</button>
            </div>
          </div>
          <p id="hubPendingRouteStatus" class="${oldStatusClass}">${oldStatus}</p>
          <p class="hub-pending-route-note">${t('pendingNote')}</p>
        </article>
      ...
      <section class="hub-pending-route-strip">
        <div><strong>${t('untilThen')}</strong><p>${t('untilText')}</p></div>
        <button type="button" class="secondary-button" data-hub-pending-overview-open>${t('backHub')}</button>
      </section>
    </div>`;
  }
  ...
  function modalShell(id, inner) {
    ...
    el.addEventListener('click', e => {
      if (e.target === el || e.target.closest('[data-v3-close]')) el.remove();
    });
    return el;
  }
  ...
  function openAccountModal() {
    ...
    $('[data-hub-account-profile]', m).onclick = () => {
      m.remove();
      showPage('profile');
      renderProfile();
    };
    $('[data-hub-account-password]', m).onclick = () => {
      m.remove();
      openPasswordModal();
    };
  ...
  function openPasswordModal() {
    ...
    $('#hubPendingPasswordForm', m).onsubmit = async e => {
      ...
      if (one.length < 8) {
        message.textContent = t('passwordShort');
        return;
      }
      if (one !== two) {
  ...
  async function copyCode() {
    ...
    if (!code || code === 'PENDING') {
      setRouteStatus(t('noCode'), 'error');
      return;
    }
    try {
      await navigator.clipboard.writeText(code);
      setRouteStatus(t('copied'), 'success');
    } catch (_) {
      setRouteStatus(t('copyFail'), 'error');
    }
  }

  async function logout() {
    try {
      if (window.HubPresenceClear) await window.HubPresenceClear();
    } catch (_) {}
    try {
  ...
  function queueSync(force = false) {
    ...
    if (!force) {
      ...
      const friends = $('#communityButton') || $('#friendsButton');
      const friendsHidden = !friends || getComputedStyle(friends).display === 'none';
  ...
  async function refresh({showMessage = false} = {}) {
    try {
      ...
      if (!session) {
        clearCache();
        clearPendingUi();
        if (location.hash === '#pending') showPage('overview');
        return;
      }
      ...
      if (cached) {
        ...
        active = true;
        sync();
        if (showMessage) setRouteStatus(t('statusError'), 'error');
      } else if (showMessage) {
        setRouteStatus(t('statusError'), 'error');
      }
    }
  }
  ...
  function handleDocumentClick(event) {
    ...
    if (target.closest?.('[data-hub-pending-status-open]')) {
      event.preventDefault();
      ensurePendingPage();
      showPage('pending');
      return;
    }
    if (target.closest?.('[data-hub-pending-overview-open]')) {
      event.preventDefault();
      showPage('overview');
      return;
    }
    if (target.closest?.('[data-hub-copy-code]')) {
      event.preventDefault();
      copyCode();
      return;
    }
    if (target.closest?.('[data-hub-check-status]')) {
      event.preventDefault();
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (route === 'pending') {
      ...
      $('#hubPendingHeaderStatus')?.classList.remove('active');
    }
    queueSync(true);
  });
  ...
  document.addEventListener('hub:auth-restored', event => {
    ...
    if (detail.registrationState?.status === 'verified') {
      ...
      clearCache();
      clearPendingUi();
      if (wasPending && location.hash === '#pending') {
        location.hash = '#profile';
        location.reload();
      }
      return;
