(() => {
  ...
  const COPY = {
    de: {
      ...
      statsText: 'Deine Statistiken, dein Rating und deine bisherigen Turniere werden sichtbar, sobald du verifiziert wurdest. Beim normalen 8-Zeichen-Code ist anschließend noch eine manuelle Bestätigung durch den Admin nötig.',
      viewStatus: 'VERIFIZIERUNGSSTATUS ANSEHEN',
      yourAccount: 'DEIN ACCOUNT',
      overviewTitle: 'Verifizierung ausstehend',
      overviewStrong: 'Stats werden nach der Bloxd-Verifizierung sichtbar.',
      overviewText: 'Rangliste, Cups und öffentliche Spielerprofile kannst du bereits ganz normal ansehen.',
      viewProfile: 'MEIN PROFIL ANSEHEN',
      statusTitle: 'Verifizierungsstatus anzeigen',
      blocked: 'Diese Funktion wird nach deiner Bloxd-Verifizierung freigeschaltet.'
    },
    en: {
      pending: 'VERIFICATION PENDING',
      verification: 'BLOXD VERIFICATION',
      statsHidden: 'Statistics are not visible yet',
      statsText: 'Your statistics, rating and previous tournaments become visible after verification. With the normal 8-character code, a manual confirmation by the admin is still required afterwards.',
      viewStatus: 'VIEW VERIFICATION STATUS',
      yourAccount: 'YOUR ACCOUNT',
      overviewTitle: 'Verification pending',
      overviewStrong: 'Stats become visible after Bloxd verification.',
      overviewText: 'You can already use the ranking, cups and public player profiles normally.',
      viewProfile: 'VIEW MY PROFILE',
      statusTitle: 'View verification status',
    ...
    fr: {
      ...
      statsText: 'Tes statistiques, ton classement et tes anciens tournois deviennent visibles après la vérification. Avec le code normal à 8 caractères, une confirmation manuelle par l’administrateur reste nécessaire ensuite.',
      viewStatus: 'VOIR LE STATUT DE VÉRIFICATION',
      yourAccount: 'TON COMPTE',
      overviewTitle: 'Vérification en attente',
      overviewStrong: 'Les statistiques deviennent visibles après la vérification Bloxd.',
      overviewText: 'Tu peux déjà utiliser normalement le classement, les cups et les profils publics.',
      viewProfile: 'VOIR MON PROFIL',
      statusTitle: 'Voir le statut de vérification',
      blocked: 'Cette fonction sera disponible après ta vérification Bloxd.'
    }
  };

  function language() {
    const raw = String(localStorage.getItem('hub_language') || localStorage.getItem('hubLang') || document.documentElement.lang || navigator.language || 'de').toLowerCase();
  ...
  function readCache() {
    try {
      ...
      if (cached.auth_user_id && authId && cached.auth_user_id !== authId) return null;
      return cached;
    } catch (_) {
      return null;
    }
  ...
  function writeCache(data, authUserId) {
    try {
      localStorage.setItem(CACHE_KEY, JSON.stringify({
        status: 'pending',
        auth_user_id: authUserId || storedAuthUserId() || '',
        claimed_name: data?.claimed_name || '',
        verified_name: data?.verified_name || '',
        source: data?.source || data?.verification_source || '',
        cached_at: Date.now()
      }));
    } catch (_) {}
  }
  ...
  function currentPendingState() {
    const candidate = state?.status === 'pending'
      ? state
      : (live.registrationState?.status === 'pending' ? live.registrationState : readCache());
    ...
    if (rankCard) {
      ...
      rankCard.innerHTML = `
        <div class="hub-pending-profile-card">
          <span class="hub-pending-kicker">${t('verification')}</span>
          <h2>${t('statsHidden')}</h2>
          <p>${t('statsText')}</p>
          <button type="button" class="secondary-button" data-hub-pending-status>${t('viewStatus')}</button>
        </div>`;
    }
  ...
  function renderOverviewPendingCard() {
    ...
    card.innerHTML = `
      <header class="panel-header tight">
        <div><span class="eyebrow">${t('yourAccount')}</span><h2>${t('overviewTitle')}</h2></div>
        <button class="hub-pending-info" type="button" data-hub-pending-status aria-label="${t('statusTitle')}" title="${t('statusTitle')}">i</button>
      </header>
      <div class="hub-pending-overview">
        <strong>${t('overviewStrong')}</strong>
        <p>${t('overviewText')}</p>
        <button class="secondary-button" type="button" data-hub-pending-profile>${t('viewProfile')}</button>
      </div>`;
  }

  function hideUnverifiedActions() {
  ...
  function installHeader() {
    ...
    badge.innerHTML = `<span class="hub-pending-mini-info">i</span> ${t('pending')}`;
    badge.title = t('statusTitle');
  ...
  function addStyles() {
    ...
    style.textContent = `
      .hub-pending-header-status{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(255,198,92,.28);background:rgba(255,198,92,.11);color:#ffc65c;border-radius:999px;padding:8px 11px;font:800 9px/1 Montserrat,Arial;letter-spacing:.06em;cursor:pointer;white-space:nowrap}
      ...
      .hub-pending-mini-info{width:15px;height:15px;font-size:9px}
      .hub-pending-info{width:32px;height:32px;color:#ffc65c;background:rgba(255,198,92,.08);font-size:14px;cursor:pointer}
      ...
      body.hub-pending-account [data-open-register],body.hub-pending-account .next-cup-cta,body.hub-pending-account [data-register-next-cup],body.hub-pending-account [data-next-cup-register]{display:none!important}
  ...
  function interceptPendingAction(e) {
    ...
    const sensitive = e.target.closest?.('[data-v3-friend-add],[data-friend-add],[data-v3-friend-remove],[data-v3-friend-accept],[data-v3-friend-decline],[data-open-register],.next-cup-cta,[data-register-next-cup],[data-next-cup-register],#editAvatarButton,#renameButton');
    if (sensitive) {
      e.preventDefault();
      e.stopImmediatePropagation();
  ...
  document.addEventListener('pointerdown', interceptPendingAction, true);
  document.addEventListener('click', interceptPendingAction, true);
  ...
  document.addEventListener('hub:auth-restored', (e) => {
    ...
    if (incoming?.status === 'pending') {
      ...
      writeCache(incoming, e.detail?.session?.user?.id || e.detail?.user?.id);
      applyUi();
      return;
    }
    if (incoming?.status === 'verified') clearCache();
